...
A personal but comprehensive Claude Code configuration, closer in spirit to agentfiles than the larger public frameworks:
...
- No equivalent of `af check` — no integrity verification
...
| Idea | How to Apply |
|------|-------------|
| **`includes/` fragments** | Add a `agentfiles/includes/` folder for per-language standards (Python: ruff/pyright rules, TypeScript: biome config, etc.) that language-expert skills inject at invocation time — keeps skill prose focused on *when* to act, not *how* to format |
| **Model preference in manifest** | `manifest.toml` `model = "opus"` field per skill; `af setup` or `af agents` surfaces which skills want a heavier model — useful when choosing between Sonnet and Opus at session start |
