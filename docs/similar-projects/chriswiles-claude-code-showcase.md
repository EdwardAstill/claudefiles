...
- **Treating config as CI** — running `cf check` (or equivalent) as a GitHub Actions step, not just a local pre-commit tool, enforces manifest/REGION.md integrity on every PR.
- **Scheduled maintenance via Actions** — dependency audit on a weekly schedule, doc freshness review monthly. Maintenance that currently requires manual `cf check` runs becomes automated.
...
- No equivalent of `cf agents` or `cf check` — discovery is manual
...
| **`cf check` as a CI step** | Add `.github/workflows/check.yml` that runs `cf check` on every PR to `claudefiles/` — catches REGION.md drift before it affects the manager | open |
...
| **`PostToolUse` auto-verification** | A hook that triggers `cf check` after any edit to a `SKILL.md` or `REGION.md` — catch integrity violations at edit time, not commit time | open |
