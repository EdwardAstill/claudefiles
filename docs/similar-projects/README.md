...
Each document covers: what the project is, what it does well, its weaknesses, and concrete ideas claudefiles could implement.
...
| [ChrisWiles/claude-code-showcase](chriswiles-claude-code-showcase.md) | Integration showcase | ~300 | Hooks × skills integration, `cf check` as CI, scheduled maintenance Actions |
...
### What claudefiles has that comparable projects don't

- Actual installer (`install.sh`) with scoped installs and granular skill/category control
- A CLI tool (`cf`) with context gathering, integrity checking, and agent inventory
...
- **Grouped `cf agents` output** — skills displayed by category namespace, not a flat dump
- **Hooks layer** — `PreToolUse` safety gate blocking dangerous Bash commands; `PostToolUse` skill logger recording every SKILL.md invocation to `~/.claude/logs/claudefiles.jsonl`
- **`cf log`** — skill usage history with per-skill filtering and frequency stats

### What comparable projects do that claudefiles currently doesn't

| Gap | Best Reference | Priority | Status |
|-----|----------------|----------|--------|
| **`SessionStart` hook** — auto-run `cf context` + `cf status` so executor skips manual orientation | hooks-mastery | High | open |
...
| **`cf check` as CI** — GitHub Actions workflow enforcing REGION.md integrity on every PR | showcase | Low | open |
