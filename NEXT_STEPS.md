...
- `iteration`: 6 / 12
- `last_tick`: iter 6 @ e5fdbdd — shipped S-3 (af check modes frontmatter validator)
...
- **⭐ NEXT  S-4** `[small]` Upgrade `research` dispatcher description to full trigger-spec (currently brief prose; only dispatcher that hasn't been rewritten).
...
- **S-7** `[small]` Quote `on`/`off` levels in MODE.md frontmatter to avoid the YAML-1.1 boolean alias quirk. `af check modes` tolerates the quirk but the canonical form should be explicit strings. Affects planner, verify-first, rubber-duck, deep-research MODE.md files.
