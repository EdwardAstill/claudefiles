...
- `iteration`: 12 / 12 — STOPPED (iteration cap reached)
- `last_tick`: iter 12 @ ec39559 — shipped 5-4 (skill-logger canonical schema)
...
| `ec39559` | 5-4 | `skill-logger.py` reads canonical `tool_response` first, with legacy `tool_output`/`output` fallbacks (iter 12) |
...
- **2026-04-19 run (iter 12/12)** — stopped on iteration cap. Shipped
  12 backlog items across 12 iterations: 3-4, 3-3, 3-5, S-1..S-6, 5-1,
  5-3, 5-4. Remaining §3 entries are all `[needs-human]` or tier-4
  `[speculative]`. Start commit `23589ba` → end commit `ec39559`.
