"""cf preview — serve HTML design mockups for visual comparison.
...
are recorded to events.jsonl so Claude can read the chosen design.
...
<head>
  ...
  <style>
    #_preview-bar{{position:fixed;bottom:0;left:0;right:0;
      background:rgba(0,0,0,.85);color:#fff;padding:8px 16px;
      font-size:13px;display:flex;align-items:center;gap:12px;z-index:9999;}}
    #_preview-bar button{{background:#3b82f6;color:#fff;border:none;
      padding:4px 12px;border-radius:4px;cursor:pointer;font-size:12px;}}
    #_preview-bar button:hover{{background:#2563eb;}}
  </style>
</head>
<body>
  <div id="_preview-bar">
    <strong style="opacity:.7">PREVIEW</strong>
    <span style="flex:1">{filename}</span>
    <button onclick="recordChoice('{filename}')">&#10003; Select this design</button>
    <span id="_status" style="opacity:.5;font-size:11px">watching...</span>
  </div>
  <div style="padding-bottom:48px">{content}</div>
  <script>
    function recordChoice(n){{
      fetch('/choice',{{method:'POST',headers:{{'Content-Type':'application/json'}},
        body:JSON.stringify({{choice:n,type:'select'}})}})
      .then(()=>{{document.getElementById('_status').textContent='✓ '+n;}});
    }}
    const _es=new EventSource('/events');
    _es.onmessage=e=>{{if(e.data==='reload')location.reload();}};
  </script>
