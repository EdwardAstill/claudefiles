"""cf tree — show folder structure as layered JSON."""
...
def main(
    ...
    write: bool = typer.Option(False, "--write", help="Save to .claudefiles/tree.json"),
):
    ...
    if output_json or write:
        ...
        if write:
            bus = Path(".claudefiles")
            bus.mkdir(exist_ok=True)
            (bus / "tree.json").write_text(out + "\n")
            typer.echo(f"Written to .claudefiles/tree.json", err=True)
        if output_json:
            typer.echo(out)
        elif not write:
            typer.echo(out)
