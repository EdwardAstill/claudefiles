...
def main(
    ...
    typer.echo(f"Open Claude Code with: claude {worktree_path}", err=True)

    # Try to launch Claude Code in a new terminal
    term = os.environ.get("TERMINAL", "")
    if term:
        try:
            subprocess.Popen([term, "-e", f"claude {worktree_path}"])
            typer.echo(f"Opened Claude Code in new terminal", err=True)
        except Exception as e:
            typer.echo(f"Could not open terminal: {e}", err=True)
    else:
        typer.echo("(No TERMINAL env set — open Claude Code manually)", err=True)
