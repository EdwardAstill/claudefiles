"""af setup — check tool dependencies for skills installed in this project."""
...
def _build_report(skill_names: list[str], skills_data: dict, cli_data: dict) -> list[str]:
    ...
    for name in sorted(skill_names):
        ...
        if tools:
            lines.append(f"  Agent tools : {', '.join(tools)}")
        if mcp_servers:
...
def main(
    write: bool = typer.Option(False, "--write", help="Write dependency report to .agentfiles/deps.md"),
    ...
    if skills:
        ...
        if not skill_names:
            ...
            typer.echo("Install skills with: af install --project")
            return
